...
# Dosya yolları (SCRIPT_DIR ve PROJECT_ROOT zaten tanımlandı)
DATA_PATH    = os.path.join(PROJECT_ROOT, "data", "master", "master_df.csv")
RESULTS_DIR  = os.path.join(PROJECT_ROOT, "src", "quanttrade", "models_2.0", "model_results_alpha_20d")

# Çalışma dizinine bağlı dosyalar (live-telegram altında)
LIVE_DIR     = os.path.dirname(SCRIPT_DIR)
STATE_PATH   = os.path.join(LIVE_DIR, "live_state_T1.json")
TRADES_CSV   = os.path.join(LIVE_DIR, "live_trades_T1.csv")
EQUITY_CSV   = os.path.join(LIVE_DIR, "live_equity_T1.csv")
EQUITY_PNG   = os.path.join(LIVE_DIR, "live_equity_T1.png")
...
def load_model_and_meta():
    ...
    if model_path is None:
        raise FileNotFoundError(f"Model dosyası bulunamadı: {RESULTS_DIR}/catboost_alpha20d_*.cbm")
...
# MAIN
# ========================
def build_text_summary(state, cash, positions, ref_data, new_signals, df):
    lines = []
    lines.append("📊 *LIVE PORTFOLIO STATE (T+1)*")
    lines.append(f"Son işlenen tarih: {state['last_date']}")
    lines.append(f"Nakit: {cash:,.2f} TL")
    lines.append(f"Aktif pozisyon sayısı: {len(positions)}")
    ...
    if positions:
        ...
        for p in positions:
            ...
            lines.append(
                ...
                f"gün sayısı {p['days_held']}"
            )
    else:
        lines.append("\nAktif pozisyon yok.")

    if new_signals:
        lines.append("\n🟢 *Yarın İçin Yeni Alım Önerileri*")
        ...
        lines.append("\n🔵 Yarın için yeni alım önerisi yok.")
...
def main():
    ...
    # Feature names varsa kullan, yoksa dataframe'den çıkar
    if not feature_names or len(feature_names) == 0:
        # select_features ile aynı logik
        drop_cols = {
        ...
        drop_cols |= {
            ...
            "alpha", "future_return", "market_future_return", SECTOR_COL
        }
        drop_cols |= {
        ...
        print(f">> Extracted {len(feature_names)} features from data")
